...
class ClusterWidget(Widget):
                ...
                for i in range(n):
                    for j in range(n):
                        ...
                        if self.percolated_cluster is not None and matriu_pintada[i, j] == self.percolated_cluster:
                            color = (0.5, random.uniform(0, 1), random.uniform(0,
                                                                               1))  # pintem de color vermell el cluster que ha percolat. Això es veu amb la segona condició del if
                        ...
                        # (vèrtex no actiu i actiu respectivament)
                            ...
                            # color = (0, 0, 0) if matriu_pintada[i, j] == 0 else (0.2 + (matriu_pintada[i,j]%10)*0.01, 0.2 + (matriu_pintada[i,j]%10)*0.08, 0.2 + (matriu_pintada[i,j]%10)*0.05)
                            color = (0, 0, 0) if matriu_pintada[i, j] == 0 else (1, 1, 1)
                        Color(*color)
                        Rectangle(pos=(j * cell_size, y_offset + self.height - (i + 1) * cell_size),
    ...
    # per afegir cel·les actives (objectiu: que el sistema percoli)
...
class PantallaTutorial(BoxLayout):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.orientation = 'vertical'
        self.add_widget(Label(text='Contingut tutorial (en constricció)'))
