...
class StartingScreen(Screen):
    def __init__(self,**kwargs):
        ...
        with self.canvas:
            self.bg = Rectangle(source='Title.png', pos=(450-179,370), size=(1196*0.3,579*0.3))
...
class TunnelingScreen(Screen):
    ...
    def send_but(self):
        ...
        for i in range(self.mlt):
            ...
            plt.plot(temps, mod2[i], label='|' + str((i - self.s)*(-1)) + '>')
        plt.legend()
...
class GameScreen(Screen):
    ...
    Comptador=None
    Counter=None
    playstate=NumericProperty(1)
    activator=None
    vb=NumericProperty(1)
    # vbut=NumericProperty(1)
    ...
    def PlayButton(self):
        if GameScreen.playstate==1: 
            GameScreen.playstate=0
        else:
            GameScreen.playstate=1
        self.play=GameScreen.playstate


    def send_but(self):
        GameScreen.vbut=self.vb
        print(self.vbut)
        plt.clf()
    ...
    # Calcul de energia no pertorbada
    def energy(self):
        ene=[]
        ...
        for i in range(len(self.temps)):
            for j in range(self.mlt):
                ene[i][j]=150+(ene[i][j]-min_ene)*250/(max_ene-min_ene)
...
class Background(Widget):
    ...
    def __init__(self,**kwargs):
        ...
        self.timer=-50
        self.counter=0
        Clock.schedule_interval(self.freshrate, 1/30)
    ...
    def figures(self,s,t,y,e):
        ...
        GameScreen.activator=1

        with self.canvas:
            self.en_t = Rectangle(source='energy.png', pos=(610, 475),size=(711*0.15,200*0.15))
            self.pro_t = Rectangle(source='prob2.png', pos=(225-90, 475),size=(1203*0.15, 200*0.15))
    ...
    def freshrate(self,dt):
        ...
        if GameScreen.playstate==0:
            GameScreen.estat=0
        if GameScreen.playstate==1 and GameScreen.activator:
            GameScreen.estat=1
        if GameScreen.estat==1:
            if GameScreen.Counter!=0:
                ...
                if self.mlt==3:
                    ...
                    m, n = self.erec1.pos
                    vel0=(self.prob[GameScreen.Comptador+1][0]*250-y0)/(self.t[GameScreen.Comptador+1]-self.timer)
                    evel0=(self.ene[GameScreen.Comptador+1][0]-n)/(self.t[GameScreen.Comptador+1]-self.timer)
                    self.rec1.size = (x0, y0 + vel0 * q * dt)
                    self.erec1.pos = (m, n + evel0 * q * dt)

                    x0, y0 = self.rec0.size
                    m, n = self.erec0.pos
                    vel0=(self.prob[GameScreen.Comptador+1][1]*250-y0)/(self.t[GameScreen.Comptador+1]-self.timer)
                    evel0=(self.ene[GameScreen.Comptador+1][1]-n)/(self.t[GameScreen.Comptador+1]-self.timer)
                    self.rec0.size = (x0, y0 + vel0 * q * dt)
                    self.erec0.pos = (m, n + evel0 * q * dt)
                    
                    x0, y0 = self.rec_1.size
                    m, n = self.erec_1.pos
                    vel0=(self.prob[GameScreen.Comptador+1][2]*250-y0)/(self.t[GameScreen.Comptador+1]-self.timer)
                    evel0=(self.ene[GameScreen.Comptador+1][2]-n)/(self.t[GameScreen.Comptador+1]-self.timer)
                    self.rec_1.size = (x0, y0 + vel0 * q * dt)
                    self.erec_1.pos = (m, n + evel0 * q * dt)
                ...
                if self.mlt==5:
                    ...
                    vel0=(self.prob[GameScreen.Comptador+1][0]*250-y0)/(self.t[GameScreen.Comptador+1]-self.timer)
                    evel0=(self.ene[GameScreen.Comptador+1][0]-n)/(self.t[GameScreen.Comptador+1]-self.timer)
                    self.rec2.size = (x0, y0 + vel0 * q * dt)
                    self.erec2.pos = (m, n + evel0 * q * dt)
                    ...
                    x0, y0 = self.rec_1.size
                    m, n = self.erec_1.pos
                    vel0=(self.prob[GameScreen.Comptador+1][3]*250-y0)/(self.t[GameScreen.Comptador+1]-self.timer)
                    evel0=(self.ene[GameScreen.Comptador+1][3]-n)/(self.t[GameScreen.Comptador+1]-self.timer)
                    self.rec_1.size = (x0, y0 + vel0 * q * dt)
                    self.erec_1.pos = (m, n + evel0 * q * dt)

                    x0, y0 = self.rec_2.size
                    m, n = self.erec_2.pos
                    vel0=(self.prob[GameScreen.Comptador+1][4]*250-y0)/(self.t[GameScreen.Comptador+1]-self.timer)
                    evel0=(self.ene[GameScreen.Comptador+1][4]-n)/(self.t[GameScreen.Comptador+1]-self.timer)
                    self.rec_2.size = (x0, y0 + vel0 * q * dt)
                    self.erec_2.pos = (m, n + evel0 * q * dt)
                ...
                if self.mlt==4:                    
                    ...
                    vel0=(self.prob[GameScreen.Comptador+1][0]*250-y0)/(self.t[GameScreen.Comptador+1]-self.timer)
                    evel0=(self.ene[GameScreen.Comptador+1][0]-n)/(self.t[GameScreen.Comptador+1]-self.timer)
                    self.rec32.size = (x0, y0 + vel0 * q * dt)
                    self.erec32.pos = (m, n + evel0 * q * dt)

                    x0, y0 = self.rec12.size
                    m, n = self.erec12.pos
                    vel0=(self.prob[GameScreen.Comptador+1][1]*250-y0)/(self.t[GameScreen.Comptador+1]-self.timer)
                    evel0=(self.ene[GameScreen.Comptador+1][1]-n)/(self.t[GameScreen.Comptador+1]-self.timer)
                    self.rec12.size = (x0, y0 + vel0 * q * dt)
                    self.erec12.pos = (m, n + evel0 * q * dt)

                    x0, y0 = self.rec_12.size
                    m, n = self.erec_12.pos
                    vel0=(self.prob[GameScreen.Comptador+1][2]*250-y0)/(self.t[GameScreen.Comptador+1]-self.timer)
                    evel0=(self.ene[GameScreen.Comptador+1][2]-n)/(self.t[GameScreen.Comptador+1]-self.timer)
                    self.rec_12.size = (x0, y0 + vel0 * q * dt)
                    self.erec_12.pos = (m, n + evel0 * q * dt)
                    
                    x0, y0 = self.rec_32.size
                    m, n = self.erec_32.pos
                    vel0=(self.prob[GameScreen.Comptador+1][3]*250-y0)/(self.t[GameScreen.Comptador+1]-self.timer)
                    evel0=(self.ene[GameScreen.Comptador+1][3]-n)/(self.t[GameScreen.Comptador+1]-self.timer)
                    self.rec_32.size = (x0, y0 + vel0 * q * dt)
                    self.erec_32.pos = (m, n + evel0 * q * dt)
                ...
                if self.mlt==6:
                    ...
                    vel0=(self.prob[GameScreen.Comptador+1][0]*250-y0)/(self.t[GameScreen.Comptador+1]-self.timer)
                    evel0=(self.ene[GameScreen.Comptador+1][0]-n)/(self.t[GameScreen.Comptador+1]-self.timer)
                    self.rec52.size = (x0, y0 + vel0 * q * dt)
                    self.erec52.pos = (m, n + evel0 * q * dt)
                    ...
                    x0, y0 = self.rec_52.size
                    m, n = self.erec_52.pos
                    vel0=(self.prob[GameScreen.Comptador+1][5]*250-y0)/(self.t[GameScreen.Comptador+1]-self.timer)
                    evel0=(self.ene[GameScreen.Comptador+1][5]-n)/(self.t[GameScreen.Comptador+1]-self.timer)
                    self.rec_52.size = (x0, y0 + vel0 * q * dt)
                    self.erec_52.pos = (m, n + evel0 * q * dt)
           
                
                self.timer = self.timer + q*dt
                self.time_lab.font_size='20sp'
                self.time_lab.text='TIME: '+ str("%.2f" % self.timer)

            
            if GameScreen.Counter==0:
                self.mlt=int(GameScreen.mlt)
                self.s=(self.mlt-1)/2.
                self.prob_space = 375. / self.mlt
                self.prob=[]
                pos_i=(-1000,-1000)
                self.vel=[]
                self.evel=[]
                ...
                self.max_ene=max([sublist[-1] for sublist in self.ene])
                self.min_ene=min([sublist[-1] for sublist in self.ene])
                ...
                if GameScreen.ft == 1:
                    with self.canvas:
                        ...
                        self.time_lab=Label(pos=(440, 510),size=(20,30), text='TIME: ')
                        self.time_lab.text='TIME: '+ str("%.2f" % self.timer)
                ...
                if self.mlt==3:
                    ...
                    self.rec1.pos=(55,150)
                    self.rec0.pos = (55 + 1 * self.prob_space, 150)
                    self.rec_1.pos=(55+2*self.prob_space,150)

                 
                    self.erec1.pos = (480 + 0 * self.prob_space, self.ene[0][0])
                    self.erec0.pos = (480 + 1 * self.prob_space , self.ene[0][1])
                    self.erec_1.pos = (480 + 2 * self.prob_space , self.ene[0][2])
                    
                    self.tag1.pos=(50+0*self.prob_space+self.prob_space/2.-15, 125)
                    self.tag0.pos=(50+1*self.prob_space+self.prob_space/2.-15, 125)
                    self.tag_1.pos=(50+2*self.prob_space+self.prob_space/2.-15, 125)

                    self.etag1.pos = (475 + 0 * self.prob_space - 15 + self.prob_space/2., 125)
                    self.etag0.pos = (475 + 1 * self.prob_space - 15 + self.prob_space/2., 125)
                    self.etag_1.pos = (475 + 2 * self.prob_space - 15 + self.prob_space/2., 125)
                    ...
                    # Movem aquells que volem que es mostrin
                    self.rec2.pos = (55 , 150)
                    self.rec1.pos = (55 + 1 * self.prob_space, 150)
                    self.rec0.pos = (55 + 2 * self.prob_space, 150)
                    self.rec_1.pos = (55 + 3 * self.prob_space, 150)
                    self.rec_2.pos = (55 + 4 * self.prob_space, 150)

                    self.erec2.pos = (480 + 0 * self.prob_space ,  self.ene[0][0])
                    self.erec1.pos = (480 + 1 * self.prob_space ,  self.ene[0][1])
                    self.erec0.pos = (480 + 2 * self.prob_space ,  self.ene[0][2])
                    self.erec_1.pos = (480 + 3 * self.prob_space , self.ene[0][3])
                    self.erec_2.pos = (480 + 4 * self.prob_space , self.ene[0][4])


                    self.tag2.pos = (50 + 0 * self.prob_space + self.prob_space / 2. - 15, 125)
                    self.tag1.pos = (50 + 1 * self.prob_space + self.prob_space / 2. - 15, 125)
                    self.tag0.pos = (50 + 2 * self.prob_space + self.prob_space / 2. - 15, 125)
                    self.tag_1.pos = (50 + 3 * self.prob_space + self.prob_space / 2. - 15, 125)
                    self.tag_2.pos = (50 + 4 * self.prob_space + self.prob_space / 2. - 15, 125)

                    self.etag2.pos = (475 + 0 * self.prob_space + self.prob_space/2. - 15, 125)
                    self.etag1.pos = (475 + 1 * self.prob_space + self.prob_space/2. - 15, 125)
                    self.etag0.pos = (475 + 2 * self.prob_space + self.prob_space/2. - 15, 125)
                    self.etag_1.pos = (475 + 3 * self.prob_space + self.prob_space/2. - 15, 125)
                    self.etag_2.pos = (475 + 4 * self.prob_space + self.prob_space/2. - 15, 125)
                    ...
                    # Movem aquells que volem que es mostrin
                    self.rec32.pos = (55, 150)
                    self.rec12.pos = (55 + 1 * self.prob_space, 150)
                    self.rec_12.pos = (55 + 2 * self.prob_space, 150)
                    self.rec_32.pos = (55 + 3 * self.prob_space, 150)

                    self.erec32.pos = (480 + 0 * self.prob_space , self.ene[0][0] )
                    self.erec12.pos = (480 + 1 * self.prob_space , self.ene[0][1] )
                    self.erec_12.pos = (480 + 2 * self.prob_space , self.ene[0][2] )
                    self.erec_32.pos = (480 + 3 * self.prob_space , self.ene[0][3] )

                    self.tag32.pos = (50 + 0 * self.prob_space + self.prob_space / 2. - 15, 125)
                    self.tag12.pos = (50 + 1 * self.prob_space + self.prob_space / 2. - 15, 125)
                    self.tag_12.pos = (50 + 2 * self.prob_space + self.prob_space / 2. - 15, 125)
                    self.tag_32.pos = (50 + 3 * self.prob_space + self.prob_space / 2. - 15, 125)

                    self.etag32.pos = (475 + 0 * self.prob_space + self.prob_space/2. - 15, 125)
                    self.etag12.pos = (475 + 1 * self.prob_space + self.prob_space/2. - 15, 125)
                    self.etag_12.pos = (475 + 2 * self.prob_space + self.prob_space/2. - 15, 125)
                    self.etag_32.pos = (475 + 3 * self.prob_space + self.prob_space/2. - 15, 125)
                    ...
                    # Movem aquells que volem que es mostrin
                    self.rec52.pos = (55 + 0 * self.prob_space, 150)
                    self.rec32.pos = (55 + 1 * self.prob_space, 150)
                    self.rec12.pos = (55 + 2 * self.prob_space, 150)
                    self.rec_12.pos = (55 + 3 * self.prob_space, 150)
                    self.rec_32.pos = (55 + 4 * self.prob_space, 150)
                    self.rec_52.pos = (55 + 5 * self.prob_space, 150)

                    self.erec52.pos = (480 + 0 * self.prob_space, self.ene[0][0] )
                    self.erec32.pos = (480 + 1 * self.prob_space, self.ene[0][1] )
                    self.erec12.pos = (480 + 2 * self.prob_space, self.ene[0][2] )
                    self.erec_12.pos = (480 + 3 * self.prob_space, self.ene[0][3] )
                    self.erec_32.pos = (480 + 4 * self.prob_space, self.ene[0][4] )
                    self.erec_52.pos = (480 + 5 * self.prob_space, self.ene[0][5] )
                    ...
                    self.etag52.pos = (475 + 0 * self.prob_space + self.prob_space/2. - 15, 125)
                    self.etag32.pos = (475 + 1 * self.prob_space + self.prob_space/2. - 15, 125)
                    self.etag12.pos = (475 + 2 * self.prob_space + self.prob_space/2. - 15, 125)
                    self.etag_12.pos = (475 + 3 * self.prob_space + self.prob_space/2. - 15, 125)
                    self.etag_32.pos = (475 + 4 * self.prob_space + self.prob_space/2. - 15, 125)
                    self.etag_52.pos = (475 + 5 * self.prob_space + self.prob_space/2. - 15, 125)
