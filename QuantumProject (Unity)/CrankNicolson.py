...
def psi_0_harm(x,y,a,b,m,w,hbar):
    ...
    psi0=c*(np.exp(-(m*w*((x-2.)**2+(y-2.)**2))/(2.*hbar))) 
    return psi0

#Funció d'ona pels 3 primers estats
def psi_harm(n,x,y,a,b,m,w,hbar):
    c=(((m*w)/(hbar*math.pi))**(1./2.))/((math.factorial(n))*(2.**n))
    if n==1:
...
def psigauss(x,y,a,b,m,w,hbar):
    psig=(1./(math.sqrt(math.pi)*1.5))*np.exp(1j*math.sqrt(2.*w)*((x)+(y))-(1./(2.*((1.5)**2)))*((x)**2+(y)**2))
    return psig


def gauss(x,y,a,b,m,w,hbar):
    sig=dispersiox(-a,-b,norm)
    p0x=0.25
    p0y=0.25
    g=(1./math.sqrt(2.*math.pi*(sig)))*math.exp(-((x**2+y**2)/(4.*(sig)))+1j*((p0x*x+p0y*y)/(hbar)))
    return g
...
#Per n=1
psiini=np.array([[psigauss(-L+i*deltax,-L+j*deltay,L,L,m,w,hbar) for i in range(Nx+1)]
	for j in range(Nx+1)],dtype=np.complex128)

Pharm=np.array([[Vll(-L+i*deltax,-L+j*deltax,L,L,m,w) for i in range(Nx+1)]
	for j in range(Nx+1)])
...
np.save('normagaussambdispersiodx{}dt{}.npy'.format(deltax,deltat),normalitzacio)	
np.save('psigaussambdispersiodx{}dt{}.npy'.format(deltax,deltat),psivector)
np.save('tvecgaussambdispersiodx{}dt{}.npy'.format(deltax,deltat),tvec)
np.save('Vvecgaussambdispersiodx{}dt{}.npy'.format(deltax,deltat),Pvec)
