...
class Inici(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        intruccions = Button(text='Instruccions', font_size=24, size_hint=(0.5, 0.25), pos_hint={'center_x': 0.5})
        intruccions.bind(on_press=self.mostrar_instruccions)
        self.add_widget(intruccions)

        tutorial = Button(text='Tutorial', font_size=24, size_hint=(0.5, 0.25), pos_hint={'center_x': 0.5})
        tutorial.bind(on_press=self.tutorial)
        self.add_widget(tutorial)

        efecte_tunel = Button(text='Efecte túnel: explicació científica', font_size=24, size_hint=(0.5, 0.25), pos_hint={'center_x': 0.5})
        efecte_tunel.bind(on_press=self.efecte_tunel)
        self.add_widget(efecte_tunel)

        jocs = Button(text='Jocs', font_size=24, size_hint=(0.5, 0.25), pos_hint={'center_x': 0.5})
    ...
    def mostrar_instruccions(self, instance):
        instruccions = Label(text='L\'objectiu del joc és sortir del laberint.\n'
                 'Per sortir del laberint, la partícula s\'ha de trobar en la casella vermella.\n'
                 ...
                 'Prem "Mesura" per a mesurar la posició exacta de la partícula.\n',
             halign='justify'
                 )
    ...
    def efecte_tunel(self, instance):
        explicacio = Label(text='L\'efecte túnel és un dels fenòmens més fascinants del món quàntic: permet a les partícules passar a través de barreres de potencial que clàssicament no podrien superar.\n\n'
                   'Imaginem-nos una bola atrapada dins un recipient (com un bol). '
                   'En la mecànica clàssica, la bola només pot sortir del recipient si té prou energia per pujar fins a la vora del recipient i sortir-ne.'
                   'Però en la mecànica quàntica, la bola pot travessar el recipient directament -com si fes un túnel a través d\'ell-, encara que no tingui prou energia per pujar fins a la vora d\'aquest.\n\n'
                   'Aquest fenomen es pot entendre com una conseqüència de la dualitat ona-partícula de la matèria, és a dir, en el món quàntic, les partícules no només es poden comportar com pilotes sòlides sinó que també es comporten com a ones.'
                   'Aquestes ones poden propagar-se a través de barreres i, per tant, hi ha una probabilitat no nul·la que les partícules es trobin en una regió on clàssicament no podrien estar.\n\n\n',
                   halign='justify')
        explicacio.bind(size=explicacio.setter('text_size'))
        imatge_imatge = Img(source='efectetunel.jpg', keep_ratio=True, allow_stretch=True, size_hint=(1, 0.85))
        referencia_imatge = Label(text='Font: https://francis.naukas.com/2009/02/04/efecto-hartman-o-la-duracion-universal-del-efecto-tunel-cuantico/')
        referencia_imatge.bind(size=referencia_imatge.setter('text_size'))
        imatge = BoxLayout(orientation='horizontal')
        imatge.add_widget(imatge_imatge)
        imatge.add_widget(referencia_imatge)
        layout = BoxLayout(orientation='vertical')
        layout.add_widget(explicacio)
        layout.add_widget(imatge)
        scrollview = ScrollView()
        scrollview.add_widget(layout)
        popup = Popup(title='Efecte túnel',
            content=scrollview,
            size_hint=(0.75, 0.75))
        popup.open()
...
class Tutorial(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.mida2 = 0.1
        self.mida = min(self.width, self.height)*self.mida2
        self.canvas.add(Color(0,0,0, 0.75))
        self.fons = Rectangle(pos=(0, 0), size=(self.width, self.height))
        self.next = Button(size_hint=(None, None), size=(self.mida, self.mida), background_normal='next_blanc.png', pos_hint={'center_x': 0.5, 'center_y': 0.4}, border=(0, 0, 0, 0), background_color=(1, 0, 0, 1))
        self.fletxa = Img(source='fletxa.png', size_hint=(0.1, 0.2), keep_ratio=True, allow_stretch=True)
        Clock.schedule_interval(self.blink_fletxa, 0.5)

        self.primer()

    def on_size(self, *args):
        self.fons.size = (self.width, self.height)
        self.fons.pos = (0, 0)
        self.mida = min(self.width, self.height)*self.mida2
        self.next.size = (self.mida, self.mida)

    def primer(self, **kwargs):
        self.canvas.add(self.fons)
        self.label1=Label(text="Recorda que l'objectiu és mesurar la partícula en el requadre vermell.")
        self.add_widget(self.label1)
        self.add_widget(self.next)
        self.next.bind(on_press=self.segon)
    
    def segon(self, instance):
        self.canvas.remove(self.fons)
        self.remove_widget(self.label1) 
        self.next.unbind(on_press=self.segon)       
        self.remove_widget(self.next)

        self.canvas.add(self.fons)
        self.add_widget(self.next)
        self.next.bind(on_press=self.tercer)
        self.label2 = Label(text='Els quadrats blancs representen els mòduls al quadrat de la funció d\'ona de la partícula, és a dir, les probabilitats de trobar la partícula en una casella en concret.', size_hint=(1, 1), text_size=(self.width*0.7, None), halign='center')
        self.add_widget(self.label2)
    
    def tercer(self, instance):
        self.canvas.remove(self.fons)
        self.remove_widget(self.label2)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.tercer)

        self.fletxa.pos_hint = {'center_x': 0.56, 'center_y': 0.84}
        self.add_widget(self.fletxa)
        self.mida2 = 0.05
        self.on_size()
        self.next.pos_hint = {'center_x': 0.56, 'center_y': 0.8}
        self.add_widget(self.next)
        self.next.bind(on_press=self.quart)

    def quart(self, instance):
        self.remove_widget(self.fletxa)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.quart)

        self.canvas.add(self.fons)
        self.label4 = Label(text='Utilitza les fletxes en pantalla o les del teclat de l\'ordinador per a moure la partícula.', size_hint=(1, 1), text_size=(self.width*0.7, None), halign='center')
        self.add_widget(self.label4)
        self.mida2 = 0.1
        self.on_size()
        self.next.pos_hint={'center_x': 0.5, 'center_y': 0.4}
        self.add_widget(self.next)
        self.next.bind(on_press=self.cinque)
    
    def cinque(self, instance):
        self.canvas.remove(self.fons)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.cinque)
        self.remove_widget(self.label4)

        self.fletxa.pos_hint = {'x': 0.27, 'y': 0.75}
        self.rot_fletxa(200)
        self.add_widget(self.fletxa)
        self.mida2 = 0.05
        self.on_size()
        self.next.pos_hint = {'center_x': 0.35, 'center_y': 0.8}
        self.add_widget(self.next)
        self.next.bind(on_press=self.sise)

    def sise(self, instance):
        self.remove_widget(self.fletxa)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.sise)

        self.canvas.add(self.fons)
        self.label6 = Label(text='Pots modificar el potencial de les "parets" del laberint, que clàssicament vindria a ser l\'alçada d\'aquestes, i el pas de temps, és a dir, cada quant de temps es deforma la funció d\'ona.', size_hint=(1, 1), text_size=(self.width*0.7, None), halign='center')
        self.add_widget(self.label6)
        self.mida2 = 0.1
        self.on_size()
        self.next.pos_hint={'center_x': 0.5, 'center_y': 0.4}
        self.add_widget(self.next)
        self.next.bind(on_press=self.sete)

    def sete(self, instance):
        self.canvas.remove(self.fons)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.sete)
        self.remove_widget(self.label6)

        self.rot_fletxa(-200)
        self.fletxa.pos_hint = {'x': 0.27, 'y': 0.55}
        self.rot_fletxa(200)
        self.add_widget(self.fletxa)
        self.mida2 = 0.05
        self.on_size()
        self.next.pos_hint = {'center_x': 0.35, 'center_y': 0.6}
        self.add_widget(self.next)
        self.next.bind(on_press=self.vuite)

    def vuite(self, instance):
        self.remove_widget(self.fletxa)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.vuite)

        self.canvas.add(self.fons)
        self.label8 = Label(text='Prem "Mesura" per a mesurar la posició exacte de la partícula.', size_hint=(1, 1), text_size=(self.width*0.7, None), halign='center')
        self.add_widget(self.label8)
        self.mida2 = 0.1
        self.on_size()
        self.next.pos_hint={'center_x': 0.5, 'center_y': 0.4}
        self.add_widget(self.next)
        self.next.bind(on_press=self.nove)

    def nove(self, instance):
        self.canvas.remove(self.fons)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.nove)
        self.remove_widget(self.label8)

        self.rot_fletxa(-200)
        self.fletxa.pos_hint = {'x': 0.27, 'y': 0.24}
        self.rot_fletxa(200)
        self.add_widget(self.fletxa)
        self.mida2 = 0.05
        self.on_size()
        self.next.pos_hint = {'center_x': 0.35, 'center_y': 0.3}
        self.add_widget(self.next)
        self.next.bind(on_press=self.dese)
    
    def dese(self, instance):
        self.remove_widget(self.fletxa)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.dese)

        self.canvas.add(self.fons)
        self.label10 = Label(text='Prem "Temps" per a fer evolucionar la funció d\'ona.', size_hint=(1, 1), text_size=(self.width*0.7, None), halign='center')
        self.add_widget(self.label10)
        self.mida2 = 0.1
        self.on_size()
        self.next.pos_hint={'center_x': 0.5, 'center_y': 0.4}
        self.add_widget(self.next)
        self.next.bind(on_press=self.onze)

    def onze(self, instance):
        self.canvas.remove(self.fons)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.onze)
        self.remove_widget(self.label10)

        self.rot_fletxa(-200)
        self.fletxa.pos_hint = {'x': 0.27, 'y': 0.15}
        self.rot_fletxa(200)
        self.add_widget(self.fletxa)
        self.mida2 = 0.05
        self.on_size()
        self.next.pos_hint = {'center_x': 0.35, 'center_y': 0.22}
        self.add_widget(self.next)
        self.next.bind(on_press=self.dotze)
    
    def dotze(self, instance):
        self.remove_widget(self.fletxa)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.dotze)

        self.canvas.add(self.fons)
        self.label12 = Label(text='Prem "Inici" per a tornar a la pantalla inicial.', size_hint=(1, 1), text_size=(self.width*0.7, None), halign='center')
        self.add_widget(self.label12)
        self.mida2 = 0.1
        self.on_size()
        self.next.pos_hint={'center_x': 0.5, 'center_y': 0.4}
        self.add_widget(self.next)
        self.next.bind(on_press=self.tretze)

    def tretze(self, instance):
        self.canvas.remove(self.fons)
        self.remove_widget(self.label12)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.tretze)

        self.rot_fletxa(-200)
        self.fletxa.pos_hint = {'x': 0.27, 'y': 0.05}
        self.rot_fletxa(200)
        self.add_widget(self.fletxa)
        self.mida2 = 0.05
        self.on_size()
        self.next.pos_hint = {'center_x': 0.35, 'center_y': 0.12}
        self.add_widget(self.next)
        self.next.bind(on_press=self.catorze)
    
    def catorze(self, instance):
        self.remove_widget(self.fletxa)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.catorze)

        self.canvas.add(self.fons)
        self.label14 = Label(text='Ara ja estàs preparat per a començar a jugar, som-hi!', size_hint=(1, 1), text_size=(self.width*0.7, None), halign='center')
        self.add_widget(self.label14)
        self.mida2 = 0.1
        self.on_size()
        self.next.pos_hint={'center_x': 0.5, 'center_y': 0.4}
        self.add_widget(self.next)
        self.next.bind(on_press=self.quinze)

    def quinze(self, instance):
        self.canvas.remove(self.fons)
        self.remove_widget(self.label14)
        self.remove_widget(self.next)
        self.next.unbind(on_press=self.quinze)


    def blink_fletxa(self, dt):
        self.fletxa.opacity = 0 if self.fletxa.opacity == 1 else 1

    def rot_fletxa(self, theta):
        pos_x = self.width * self.fletxa.pos_hint['x']
        pos_y = self.height * self.fletxa.pos_hint['y']
        with self.fletxa.canvas.before:
            PushMatrix()
            Rotate(angle=theta, origin=(pos_x+self.fletxa.width/2, pos_y+self.fletxa.height/2))
        with self.fletxa.canvas.after:
            PopMatrix()
