...
def evolution(state):
    ...
    temp1=state
    maximum=max(temp1)
    index=np.where(temp1 == maximum)
    temp1=np.delete(temp1, index)
    maximum2=max(temp1)
    phase=[]
    for j in range(len(state)):
        if ((np.absolute(maximum2)-np.absolute(state[j]))/np.absolute(maximum2)) < 0.4:
    ...
    for i in range(steps):
        ...
        if count%save==0:
            ...
            temp=system
            maximum=max(temp)
            index=np.where(temp == maximum)
            temp=np.delete(temp, index)
            maximum2=max(temp)
            phases=[]
            for j in range(len(system)):
                if ((np.absolute(maximum2)-np.absolute(system[j]))/np.absolute(maximum2)) < 0.4:
...
def initialize(self):
    ...
    text1=fig.text(0.01,0.75,'$v_1$=' + str(v1), fontsize=10)
    text2=fig.text(0.01,0.7,'$v_2$=' + str(v2), fontsize=10)
    text3=fig.text(0.01,0.65,'$z_1$=' + str(z01), fontsize=10)
    text4=fig.text(0.01,0.60,'$z_2$=' + str(z02), fontsize=10)
    text5=fig.text(0.01,0.55,'k=' + str(k), fontsize=10)
    text6=fig.text(0.01,0.5,'a=' + str(a), fontsize=10)
...
def ex1(self):
    ...
    text4=fig.text(0.01,0.60,'$z_2$=' + str(0), fontsize=10)
