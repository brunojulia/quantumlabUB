...
def Max_Boltz(ground,energy,temp):
    ...
    E=energy-ground
    kT=kb*temp
    prob=E/(np.exp((E-mu)/kT))
    return prob

@jit
def Max_Boltz_vtrue(ground,energy,temp):
    E=energy-ground
    kT=kb*temp
    prob=2*np.sqrt(E/pi)*((kT)**(-1.5))*np.exp(-E/kT)
    return prob
...
def bb_ev(bb_Egze_p012,temp):
    ...
    for pas in range(shape_array_bb[0]):
        ...
        if delta_e <= 0:
            ...
            bb_Egze_p012[i,j,4]=bb_Egze_p012[i,j,4]+\
                            energ-discrete_energy_arr(energ, 
                                                      bb_Egze_p012[i,j,2])
...
radiance_wavelenght(p_nu_l,"hst_bb_0",50,T)
