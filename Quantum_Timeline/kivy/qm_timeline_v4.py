...
class MyScreenManager(ScreenManager):
    def  __init__(self, **kwargs):
        ...
        self.get_screen("SchroMirror").smpseudo_init()
    pass
...
class DiscoveriesScreen(Screen):
    ...
    def transition_DSM(self):
        """Transició des de la pantalla Discoveries a la pantalla SchrodingerMirror"""
        
        smscreen=self.manager.get_screen('SchroMirror')
        smscreen.sm_schedule_fired()
        self.manager.transition=FadeTransition()
        self.manager.current='SchroMirror'
...
class SchroMirrorScreen(Screen):
    ...
    def smpseudo_init(self):
        ...
        self.wave_slit=self.wave_visu.imshow(self.pot.transpose()\
            [int((self.Ny-self.h_display)/2):int((self.Ny+self.h_display)/2),
             0:self.w_display],origin='lower',cmap=self.cmap,
             vmax=self.max_V,vmin=0.01*self.max_V,alpha=0.75,
        extent=(0,int(self.w_display*self.dl),0,int(self.h_display*self.dl)))
    ...
    def sm_schedule_fired(self):
        "Ara, definim l'update, a partir d'aqui farem l'animació"
        self.schedule=Clock.schedule_interval(self.plotsmev,self.frame_t)
    ...
    def plotsmev(self,dt):
        ...
        self.wave_slit=self.wave_visu.imshow(self.pot.transpose()\
            [int((self.Ny-self.h_display)/2):int((self.Ny+self.h_display)/2),
             0:self.w_display],origin='lower',cmap=self.cmap,
             vmax=self.max_V/2,vmin=0.01*self.max_V,alpha=0.75,
        extent=(0,int(self.w_display*self.dl),0,int(self.h_display*self.dl)))
    ...
    def change_y02(self,value_y02,*largs):
        self.y02=value_y02
