...
r_c=np.zeros((10,10,10))

file1=open("coeficient_de_reflexio(0.01-0.055,14-25,3.9-4.8).txt","a")
...
for i in range(10):
    for j in range(10):
        for k in range(10):
            sgm_max=0.01+0.005*i
            gruix_paret=16+j
            m=3.9+0.1*k
            r_c[i,j,k]=reflection_coefficient(sgm_max,gruix_paret,m)
